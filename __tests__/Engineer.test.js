...
let newguy = new Engineer("Buffy", "030029", "buffy@cooljob.com", "Gachnar42");
...
test("The employee should be an engineer.", () => {
    ...
    expect(newguyRole).toMatch("Engineer");
}) 

// EMPLOYEE GITHUB USERNAME
test("GitHub username should be a usable string with no spaces.", () => {
    let newguyGithub = newguy.getGithub();
    const newguyGithubChars = newguyGithub.split("");

    expect(newguyGithub).not.toBeUndefined();
    expect(newguyGithub).not.toBeNull();
    expect(newguyGithub).not.toBeFalsy();
    expect(newguyGithubChars).not.toContain(" ");
})